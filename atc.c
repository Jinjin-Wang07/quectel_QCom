...
static int atc_init(PROFILE_T *profile) {
    ...
    if (!at_response_error(err, p_response)) {
        ...
        if (err == 2 && old_nat != new_nat) {
            safe_at_response_free(p_response);
            if (asprintf(&cmd, "AT+QCFG=\"NAT\",%d", new_nat) < 0) {
                err = -1;
                goto exit;
            }
...
static int requestEnterSimPin(const char *pPinCode) {
    ...
    if (asprintf(&cmd, "AT+CPIN=%s", pPinCode) < 0)
        return retVal;
...
static int requestSetProfile(PROFILE_T *profile) {
    ...
    if (asprintf(&cmd, "AT+QICSGP=%d,%d,\"%s\",\"%s\",\"%s\",%d",
        profile->pdp, profile->iptype, new_apn, new_user, new_pd, profile->auth) < 0)
        return -1;
    err = at_send_command(cmd, &p_response);
    safe_free(cmd);
    if (at_response_error(err, p_response)) {
        safe_at_response_free(p_response);
        if (asprintf(&cmd, "AT+CGDCONT=%d,\"%s\",\"%s\"", profile->pdp, ipStr[profile->iptype], new_apn) < 0)
            return -1;
        err = at_send_command(cmd, &p_response);
        safe_free(cmd);
    }
...
_re_check:
    if (asprintf(&cmd, "AT+QICSGP=%d", profile->pdp) < 0) {
        err = -1;
        goto _error;
    }
    ...
    if (err == AT_ERROR_INVALID_RESPONSE && p_response == NULL) {
        //bug of RG801H
        safe_at_response_free(p_response);
        if (asprintf(&cmd, "AT+QICSGP=%d,%d,\"\",\"\",\"\",0", profile->pdp, profile->iptype) < 0) {
            err = -1;
            goto _error;
        }
        err = at_send_command(cmd, &p_response);
...
static int requestSetupDataCall(PROFILE_T *profile, int curIpFamily) {
    ...
    if (strStartsWith(profile->BaseBandVersion, "RG801H") || strStartsWith(profile->BaseBandVersion, "EC200H")) {
        ...
        if (asprintf(&cmd, "ifconfig %s up", profile->usbnet_adapter) < 0)
            goto _error;
        if (system(cmd)) {};
        safe_free(cmd);
    }
    ...
    if (asr_style_atc) {
        ...
        if (state == 0) {
            if (asprintf(&cmd, "AT+CGACT=1,%d", pdp) < 0)
                goto _error;
            err = at_send_command(cmd, &p_response);
            safe_free(cmd);
            if (at_response_error(err, p_response))
    ...
    if(asr_style_atc) {
        if (asprintf(&cmd, "AT+QNETDEVCTL=1,%d,%d", pdp, 1) < 0)
            goto _error;
    } else {
        if (asprintf(&cmd, "AT+QNETDEVCTL=%d,1,%d", pdp, 1) < 0)
            goto _error;
    }
    err = at_send_command(cmd, &p_response);
    safe_free(cmd);
    ...
    if (!asr_style_atc) { //TODO some modems do not sync return setup call resule
        ...
        while (t++ < 15) {
            if (asprintf(&cmd, "AT+QNETDEVSTATUS=%d", pdp) < 0)
                goto _error;
...
static int requestDeactivateDefaultPDP(PROFILE_T *profile, int curIpFamily) {
    ...
    if (asr_style_atc) {
        if (asprintf(&cmd, "AT+QNETDEVCTL=0,%d,%d", pdp, 0) < 0)
            return -1;
    } else {
        if (asprintf(&cmd, "AT+QNETDEVCTL=%d,0,%d", pdp, 0) < 0)
            return -1;
    }
...
static int requestGetIPAddress(PROFILE_T *profile, int curIpFamily) {
    ...
    if (asprintf(&cmd, "AT+CGPADDR=%d", profile->pdp) < 0) {
        err = -1;
        goto _error;
    }
